...
不采用"多 Agent 自由协作"的复杂架构，而是：**一个中心编排器 + 多个独立 Stage**。Stage 之间不直接通信，仅通过工作区文件交换信息。
...
每个 Stage 的输入输出都是明确定义的文件：

| Stage | 输入文件 | 输出文件 |
|-------|----------|----------|
| Ingest | `problem.pdf` | `problem_context.md`, `constraint_hints.json` |
| TestGen | `problem_context.md` | `generated_cases.json` |
| Solver | `problem_context.md`, `merged_cases.json`, `demo_observations.json` | `candidates/v1/*.cpp`, `solver_notes.json` |
| Evaluator | 候选代码 + 测试用例 | `evaluation_summary.json`, `compare_result.json` |
| Humanizer | 候选代码 + 风格警告 | `humanized/*.cpp` |
...
评测失败时，提取结构化摘要（而非完整日志）反馈给 Solver：
- 编译错误摘要
...
│              主编排器：调度各 Stage，管理全局状态             │
└──────┬─────────┬─────────┬─────────┬─────────┬──────────────┘
       │         │         │         │         │
       ▼         ▼         ▼         ▼         ▼
   ┌──────┐  ┌──────┐  ┌──────┐  ┌──────┐  ┌──────────┐
   │Ingest│  │TestGen│  │Solver│  │Eval  │  │Humanizer │
   └──────┘  └──────┘  └──────┘  └──────┘  └──────────┘
       │         │         │         │         │
       └─────────┴─────────┴─────────┴─────────┘
                       │
                       ▼
              ┌─────────────────┐
              │  CodexAdapter   │  ← 封装 Codex CLI 调用
              └─────────────────┘
```

### 3.1 核心组件

#### Orchestrator (`AutomaticGC` 类)
...
- 封装 `codex exec` 命令行调用
- 提供 JSON Schema 约束的返回格式
- Prompt 模板渲染（读取 `docs/prompts/*.md`）
...
### 6.1 为什么不用真正的 Multi-Agent 架构？

原始需求提到 4 个 Agent（解题、造数据、校验、风格调整），但实际实现采用**编排器 + 子任务**模式：

| 方案 | 优点 | 缺点 | 本项目的取舍 |
|------|------|------|-------------|
| 自由协作 Multi-Agent | 理论上更灵活 | 调试困难、成本高、上下文污染、难复现 | ❌ 放弃 |
...
Solver Prompt 会包含这些观察，显著提升首次通过率。
...
修改 `CodexAdapter` 为通用接口，运行时通过配置切换。
...
4. **Humanizer 风险**：风格降级可能引入隐性 bug，虽已做回归验证但仍非 100% 安全

---

## 9. 总结

auto_fuck_sj 是一个**务实的工程化方案**：用简单的编排器架构 + 文件接口，串联起 AI 代码生成和 Windows 本地工具链。它没有追求复杂的 Multi-Agent 理论模型，而是以"最快跑通、最易调试"为首要目标，在实际课程作业场景中达到了可用的自动化水平。
